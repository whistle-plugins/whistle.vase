...
function type(ctnType) {
  return setType(function() {
    return typeof ctnType === 'string' ? ctnType.trim() : '';
  }, 'type');
}
...
module.exports = function(script, req, callback) {
  ...
  outerList.forEach(function(outer) {
    ...
    case 'type':
      if (value) {
        var newType = value.split(';');
        var type = newType[0];
        newType[0] = !type || type.indexOf('/') != -1
            ? type : (type === 'sse' ? 'text/event-stream' : mime.lookup(type, type));
        headers['content-type'] = newType.join(';');
      }
      break;
    default:
      bodyList.push(outer);
