...
function out(data, delay, speed) {
	var delay = delay && parseInt(delay, 10) || 0;
	var speed = speed && parseFloat(speed, 10) || 0;
...
function json(data) {
	return setType(function() {
		...
		resolveValue(data).then(function(value) {
			deferred.resolve(util.parseJSON(value) || '');
		});
		return deferred.promise; 
	}, 'json');
}
	...
	function outerHandler() {
		...
		Q.all(bodyList.map(function(args) {
			...
			for (var i = 0, len = list.length; i < len; i++) {
				...
				bodyList[i] = {
								content: Buffer.isBuffer(value) ? value : new Buffer(value),
								delay: args[1],
